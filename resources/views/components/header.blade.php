        ...
        <div class="relative" x-data="{ isOpen: false}">
            <div class="flex items-center cursor-pointer py-2" @mouseEnter="isOpen = true" @mouseLeave="isOpen = false">
                <p class="hidden md:block text-md font-semibold mr-4 text-nu-primary">
                    {{ Auth::user()->getIsStudentAttribute() ? Auth::user()->email : __('Log out') }}</p>
                @if (Auth::user()->getIsStudentAttribute())
                    <button class="bg-white p-3 lg:py-4 shadow-drop rounded-md" @keydown.enter="isOpen = true"
                        @keydown.escape="isOpen = false">
                        <div class="hidden lg:block transform transition-transform duration-200 text-nu-primary"
                    ...
                    <ul x-cloak x-show=" isOpen"
                        class="absolute font-normal bg-white shadow-drop rounded-md overflow-hidden w-48 right-0 z-20 top-full">
                        <li>
                        ...
                        <li>
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <a href="{{ route('logout') }}"
                                    ...
                                    </svg><span class="ml-2 text-sm font-medium">{{ __('Log out') }}</span>
                                </a>
                            </form>
                        </li>
                    </ul>
                @else
                    <a href="{{ route('logout') }}" class="bg-white p-3 lg:py-4 shadow-drop rounded-md">
